...
- detect_duplicates_for_workspace(connection, workspace_id)
  ...
  means an already-detected pair is never re-inserted.
...
Detected pairs are never merged or auto-resolved by this module - they're
purely a report (see duplicates.service.js#listDuplicateGroups, which
groups transitively-connected pairs for display). Both exclude pairs whose
two slots already share the same content_id (migration 030) - if a
Question Bank copy has already repointed one slot's content onto
another's, their question_text is trivially identical (100% similarity)
and re-flagging that on every subsequent run would just be noise, not a
genuine finding.
"""
...
def _insert_candidate_pairs(connection, query, params):
    ...
    don't count - ON CONFLICT DO NOTHING is silent).
    """
...
def detect_duplicates_for_mock_test(
    ...
    an accidental duplicate worth surfacing than two questions that
    arrived from separate uploads.
    """
    query = """
