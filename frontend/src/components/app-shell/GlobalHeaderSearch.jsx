...
export default function GlobalHeaderSearch({ mobile = false }) {
  ...
  useEffect(() => {
    ...
    const handleClickOutside = (e) => {
      if (
        ...
        !desktopContainerRef.current.contains(e.target)
      ) {
        setIsOpen(false);
      }
    };
  ...
  const renderResultsContent = () => {
    ...
    return (
      <div className="overflow-y-auto divide-y divide-border p-2 space-y-2">
        {clusterResults.length > 0 && (
          <div>
    ...
    return (
      <>
        <button
          ...
          className="h-9 w-9 rounded-md border border-border flex items-center justify-center text-muted-foreground hover:text-foreground transition-all"
        >
        ...
        {isOpen &&
          createPortal(
            <div className="fixed inset-0 z-50 bg-background/95 backdrop-blur-md flex flex-col p-4 animate-in fade-in-0 duration-150">
              <div className="flex items-center gap-2 mb-3">
                <div className="relative flex-1 flex items-center">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-10 py-2.5 text-sm rounded-md border border-border bg-card focus:outline-none focus:ring-2 focus:ring-orange-500/30 text-foreground placeholder:text-muted-foreground"
                  />
                  {query && (
                    <button
                      type="button"
                ...
                <button
                  ...
                  className="px-3 py-2 border border-red-500/50 text-xs font-semibold text-red-500/50 hover:text-foreground rounded-full transition-colors"
                >
                  Cancel
                </button>
              </div>

              <div className="flex-1 overflow-y-auto surface-card rounded-2xl border border-border shadow-2xl">
