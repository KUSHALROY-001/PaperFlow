...
function canRevealRealAnswer(question) {
  return (
    question.gradingStatus !== "pending_grading" && hasRealAnswer(question)
  );
}
      ...
      <div className="w-full max-w-5xl space-y-6">
        <div className="surface-card rounded-3xl p-2  md:p-6 border border-border text-center">
          ...
          <p className="text-muted-foreground text-xs font-normal mb-8">
            Score: {attempt.score}
            {maxMarks != null ? ` / ${maxMarks}` : ""} marks (negative marking
            applied)
          </p>
          ...
          {showSaveResultBanner && claimStatus !== "saved" && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mb-8 px-4 py-3.5 rounded-2xl border border-blue-500/20 bg-blue-500/10 text-left">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-medium rounded-full text-xs sm:text-sm shrink-0 disabled:opacity-60"
              >
          ...
          <div className="flex gap-3 justify-center">
            {isGuest ? (
              <a
                href="/"
                className="flex items-center gap-2 px-6 py-3 rounded-full border border-black dark:border-white bg-transparent hover:bg-black dark:hover:bg-white text-foreground hover:text-white dark:hover:text-black shadow-xs transition-all text-sm"
              >
                Try PaperFlow free →
              </a>
            ) : (
              <>
                ...
                <Link
                  to="/clusters"
                  className="flex items-center gap-2 px-6 py-3 border border-black dark:border-white bg-transparent hover:bg-black dark:hover:bg-white text-foreground hover:text-white dark:hover:text-black rounded-md shadow-xs transition-all text-sm"
                >
                  View Clusters
                </Link>
              </>
