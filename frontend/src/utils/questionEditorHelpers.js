...
export function toEditorQuestion(question) {
  ...
  return {
    ...
    // <img .../>}` check, even though the API response has it. Also what
    // DiagramCropModal now crops directly against - there's no longer a
    // separate diagramOriginalUrl (see migration
    // 022_diagram_single_image.sql: one stored image per diagram, not
    // two, so a re-crop starts from the previous crop's result).
    ...
    // attachDiagramSource (editor-only, mock-tests.service.js#listQuestions) -
    // DiagramUploadControl's replace-confirm copy treats an absent source
    // as "extracted" (nothing to distinguish yet).
